...
Sampler_tau_ind = function (i, tau.curr, n.proc, data, dist, rho, m, m.denovo, m.maint, d.denovo, d.maint, b, c, sd.tau, lower, upper, DE.NOVO=1, PRINT.FLAG=0)
{
	lambda.curr = c(as.vector (tau.curr), as.vector(rho), as.vector(m))
	lambda.new = lambda.curr
	par.new = rnorm (1, tau.curr[i], sd.tau)
	lambda.new[i] = ifelse (par.new>upper | par.new<lower, tau.curr[i], par.new)
