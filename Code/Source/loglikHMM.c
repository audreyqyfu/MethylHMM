 ...
 * loglikHMM.c
 *
 * 2011.09.11: Bug fix: some emission probabilities were wrong.
 ...
 *
 * Function to compute log likelihood of all 
 * the double-stranded sequences under the hidden 
 * Markov model.  The Markov processes are modelled 
 * as jump processes with bisulfite conversion error 
 * rates b (failure of conversion) and c (inappropriate 
 * conversion) constant across sites.
 * 
 ...
 * output: loglik: log likelihood of all the sequences
 ************************************************************/
# include <R.h>
# include <stdlib.h>
# include <stdio.h>
...
double loglikind_hmm (int ** ind_seq, double * pi, double *** A, double *** B, int length, int n_hidden, int print_flag);
...
void loglikHMM (double * loglik, int * data, double * lambda, double * MDenovo, double * MMaint, double * DDenovo, double * DMaint, double * b, double * c, int * NSeq, int * NSite, int * NHidden, int * NObs, int * NProc, int * DE_NOVO, int * PRINT_FLAG)
{
	double m_denovo = MDenovo[0];		/* de novo rate of Dnmt1 */
	double m_maint = MMaint[0];			/* rate of maintenance activity of Dnmt1 */
	double d_denovo = DDenovo[0];		/* de novo rate of Dnmt3 on daughter strand */
	double d_maint = DMaint[0];			/* rate of maintenance activity of Dnmt3 on daughter strand */
	double error_b = b[0];
  double error_c = c[0];
  ...
  pi_ind = (double *) malloc (sizeof (double)* n_proc);
	...
	if (print_flag)
	{
		...
		for (i = 0; i<n_site; i++)
			printf ("%lf ", m[i]);
		printf ("\n");
		printf("error rate b: %lf\n", error_b);
		printf("error rate c: %lf\n", error_c);
	}
  ...
  for (i=0; i<n_seq; i++)
      ...
      if (print_flag)
	{
       ...
       *****************************************/
      result += loglikind_hmm (ind_seq, pi, trans_mtx, emiss_mtx, n_site, n_hidden, print_flag);
      ...
      if (DE_NOVO==0)
	{
	  ...
	  emiss_mtx[7][3] = 1.0;
		...
		if (DE_NOVO==1)
		{
			...
			emiss_mtx[4][2] = m_s * (1.0-m_maint);
			emiss_mtx[4][3] = m_s * m_maint;
			emiss_mtx[5][0] = (1.0-m_s) * (1.0 - m_denovo) * (1.0-d_denovo);
			emiss_mtx[5][1] = (1.0-m_s) * (m_denovo + d_denovo - m_denovo * d_denovo);
			emiss_mtx[5][2] = m_s * (1.0 - m_maint) * (1.0-d_maint);
			emiss_mtx[5][3] = m_s * (m_maint + d_maint - m_maint * d_maint);
			emiss_mtx[6][2] = (1.0 - m_s) * (1.0 - m_denovo) + m_s * (1.0 - m_maint);
			emiss_mtx[6][3] = (1.0 - m_s) * m_denovo + m_s * m_maint;
			emiss_mtx[7][2] = m_s*(1.0-m_maint)*(1.0-d_maint) + (1.0-m_s)*(1.0-m_denovo)*(1.0-d_denovo);
			emiss_mtx[7][3] = m_s*(m_maint+d_maint-m_maint*d_maint) + (1.0-m_s)*(m_denovo+d_denovo-m_denovo*d_denovo);
		}
 ...
 * loglikind_hmm
 *
 * function to compute log likelihood of an 
