##########################################################################
# StrandAssignmentProbs
#
# 2009.08.09: parametrization as in Note_10_EmissProbs
# 2009.07.26: file name and function name changed.
# 2008.09.07: compute strand type prob for each sequence 
#
# wrapper function that calls the C subroutine "StrandAssignmentProbs" 
# to compute strand assignment probabilities for all double-stranded sequences
# under the hidden Markov model
#
# input: data
#        lambda: tau, vector of length 3; rho, vector of length 3; m of length n.site
#		 dist: pairwise distance in nt
...
# output: strand assignment probs: probs of top strand being parent; vector of length S
##########################################################################

StrandAssignmentProbs = function (data, lambda, dist, m.denovo, m.maint, d.denovo, d.maint, error.b, error.c, DE.NOVO=1, PRINT.FLAG=0)
{
...
#	print (lambda)
	...
	out=.C("StrandAssignmentProbs", strandtype=as.double(rep(0,N)), data.tmp, lambda.tmp, m.denovo.tmp, m.maint.tmp, d.denovo.tmp, d.maint.tmp, error.b.tmp, error.c.tmp, N, S, n.hidden, n.obs, n.proc, as.integer(DE.NOVO), as.integer(PRINT.FLAG))
	return(out$strandtype)
}
